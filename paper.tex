...
\begin{abstract}
Heterogeneous security evidence can be fused into a directed graph and ranked by path score, but the confidence attached to a relationship does not establish that the relationship is supported by evidence. We report a failure analysis of our own prototype, AetherGraph, which fuses active reconnaissance, identity collection, repository secret scanning, web evidence and manual penetration-test findings, and ranks paths across evidence layers.

Under the original graph semantics, which traversed all edge types, an earlier run produced two cross-layer \emph{evidence} paths with a highest score of 0.914; under the corrected semantics these do not qualify as attack paths. Forensic reconstruction of the stored artifacts established that the relationships carrying that result were not independently supported by the collected evidence: the secret scanner emitted a fixed service target for every repository credential; the manual collector converted an analyst-supplied field directly into a graph relationship; and the identity collector created an account relationship from an email address alone, on a service provisioned for a different user. The scoring computation was internally consistent and we reproduce the 0.914 value exactly from the original stored streams. The defect lay in graph construction, not in the scoring function.

We identify a second and independent defect. Path traversal treated every edge type as attacker movement, including asset-containment and evidence-lineage relationships. A three-node evidence chain present in the corrected graph, both of whose relationships are directly observed, scores 1.354 under that traversal --- exceeding the unsupported path --- when the origin predicate is relaxed to the two origins the chain spans. This figure characterizes the traversal defect and is not a headline experimental result: under the three-origin predicate used throughout the evaluation, the corrected graph yields no path under either traversal.

The corrected implementation introduces edge provenance (observed, derived, declared), justification requirements for derived relationships, derivation rules that decline when required evidence is absent, endpoint validation that rejects edges into unemitted nodes, an edge-semantics registry restricting attack traversal to capability relationships, and a seed manifest defining controlled expectations for regression testing. The corrected pipeline recovered all five expected relationships defined by that manifest and emitted neither of the two previously identified fabrication patterns. The corrected graph contains 14 nodes and 13 edges, of which 9 are observed, 3 derived and 1 declared, and contains no capability relationship. No valid attack path was produced under the defined access-edge traversal semantics, in any provenance mode.
...
Security assessment produces evidence in incompatible silos. A port scanner reports open services, an OSINT collector reports identities, a repository scanner reports leaked secrets, and a penetration tester writes a report. Four artifacts, four formats, no shared identifiers.

Composition is where the risk lives. An open service may be routine, and a leaked token may be inert, but a leaked token that authenticates to that service is neither. Siloed tooling cannot represent this, which motivates fusing evidence into a single graph and ranking the paths through it. That idea is well established in the attack-graph literature and is the design AetherGraph implements.

The difficulty is that a fusion graph makes every relationship look alike. Once a scanner observation, a rule's inference and an analyst's assertion are all rendered as a typed arrow carrying a confidence value, no downstream computation can tell them apart. Confidence expresses how strongly a relationship is asserted. It says nothing about whether the relationship was observed, derived from evidence, or simply assumed. In our prototype this manifested concretely: a high-scoring cross-layer path was reported whose critical relationships were fixed constants in the collector source rather than conclusions drawn from evidence.

This paper asks: \emph{what must be recorded about a relationship for a ranked path over fused security evidence to be interpretable as a security finding?} We address it through a failure analysis and its correction. Two objectives follow: (i) to determine why conventional safeguards --- confidence thresholds, path-length penalties, score reproduction and unit tests --- failed to detect unsupported relationships, and (ii) to identify the minimal construction discipline that prevents their recurrence.
...
That literature largely assumes its inputs. Vulnerability and configuration facts arrive from a scanner or a database and are treated as given; the research contribution is the reasoning over them. Our failure occurred beneath that assumption, in the construction of the facts themselves, and would not have been caught by any amount of care in the reasoning layer.
...
\textbf{Evidence fusion, correlation and provenance.} Correlating vulnerability, asset and identity records into a single representation is a long-standing practice, and the separation of an assertion's strength from its evidentiary origin is a familiar idea in data provenance and in structured threat-intelligence formats, which commonly carry confidence and source fields separately. We do not claim to introduce that separation. Our narrower observation, which we support empirically rather than by appeal to the literature, is that graph-based security inference requires the distinction to be enforced \emph{per edge and at construction time}, because path scoring multiplies edge values together and thereby aggregates assumptions and measurements into a single number with no residual trace of which was which. We are not aware of prior work that reports this specific failure mode in a fusion pipeline, but we make no priority claim: our contribution is the documented failure, its diagnosis and a construction discipline, not the concept of provenance.

\textbf{Distinguishing this work.} We propose neither a new attack-graph formalism nor an improved risk metric, and we make no novelty claim regarding provenance as a concept. We report a construction-time failure mode in a fusion pipeline, show that conventional safeguards did not detect it, and describe an architectural change that prevents the identified patterns from recurring. The evaluation is a controlled negative result on a single synthetic testbed rather than a performance claim.
...
These three levels are an operational taxonomy chosen for this prototype to separate direct observation, deterministic evidence-based derivation, and explicit human assertion. We do not claim the taxonomy is theoretically complete, minimal, or universally correct; finer distinctions (for example between tool classes of differing reliability, or between derivation rules of differing strength) are plainly possible. It is a design choice that served the failure analysed here, and it requires broader validation before being recommended generally. A stream written before provenance labelling loads as \emph{declared}, the most conservative reading, so a legacy artifact can be compared against without being promoted to discovered.
...
Only \emph{access} edges are walked when inferring an attack path, in the sense fixed in Section~\ref{sec:terminology}; that is, $\mathit{ATTACK\_TRAVERSABLE} = \{\textsc{access}\}$. This is a prototype operational definition and a controlled design decision, not a claimed universal ontology of attack relationships: the assignment of the nine edge types to five categories reflects our judgement about this schema and has not been externally validated. Widening the traversable set is a research decision requiring argument, not a convenience. We retain the ability to traverse the full evidence graph, because evidence-graph connectivity is a legitimate question; it is simply a different one, and Section~\ref{sec:eval} reports the two separately.
...
Rules record why they declined, which is reported at run time and is how a reader distinguishes "no relationship exists" from "the collector was silent".
...
We verified the implementation against both Equation~\eqref{eq:cr} and Equation~\eqref{eq:path}. The weights are fixed in source as stated; $\lambda=0.95$; the confidence floor is $0.60$ and rejects any edge below it; and $|\pi|$ counts \emph{nodes}, so the length bound of 5 admits at most 4 edges. Severity absent from a record and severity present but explicitly null are both normalized to $0.0$; the latter previously raised a type error, and is now covered by a regression test. Entry nodes are those with identity or manual origin and jewels are service records: both are prototype conventions of the runner, not a validated organizational asset model, and we state them as such.
...
Forensic analysis of the collector source established that the relationships carrying that path were not independently supported by the collected evidence. We identify three distinct generation mechanisms, two of which appear in the highest-scoring path.

\textbf{The \texttt{ENABLES\_ACCESS} edge.} The secret scanner emitted, for every credential found in any repository, an edge to \texttt{svc-\{ip\}-3000}. The port was a literal in the source. The scanner found an AWS-shaped canary and linked it to a Gitea service; the two are unrelated technologies, and the seed repository has no configured remote of any kind.
...
The common cause is that each check concerned how strongly a relationship was asserted, and none concerned its evidentiary origin. A confidence of $0.75$ assigned by a scanner and a confidence of $0.75$ supplied by a human are indistinguishable to every downstream computation, and Equation~\eqref{eq:path} aggregates them multiplicatively without distinction.
...
\subsection{Tests}
The suite contains 74 tests, all passing. Thirty-five of them assert that a rule \emph{declines}, that a malformed input is \emph{rejected}, or that an identified defect \emph{cannot recur}: that a repository with no remote yields no service edge, that an incompatible secret kind yields none, that an uncorroborated username yields no account, that a report naming an unobserved port yields no validation edge, that an ambiguous host match is refused, that an edge to an unemitted node is rejected, that a derived edge without justification is rejected, that declared edges are not promoted, and that the two forbidden relationships are absent from the recorded artifacts. Tests of that shape are the ones that would have caught the original defect; the eight tests present at the time did not include any.

\section{Discussion}\label{sec:discussion}
\textbf{Confidence and provenance are orthogonal.} Confidence answers how strongly a relationship is asserted; provenance answers what its evidentiary origin and status are. The two vary independently: a relationship may be weakly asserted but directly observed, or asserted at $0.75$ and merely declared. In the original schema both were collapsed onto a single numeric channel, with the consequence that a $0.75$ observed relationship and a $0.75$ declared relationship were evidentially equivalent to every downstream computation, and Equation~\eqref{eq:path} aggregated them identically. We do not claim this as a general theorem; we claim it as a design requirement supported by the failure documented here, namely that a system ranking paths over fused heterogeneous evidence should record both properties per edge if its output is to be interpretable.
...
\textbf{Negative tests are the effective safeguard.} The defect persisted in part because the test suite verified that rules produced edges and never that they declined to do so. In an inference system whose characteristic failure mode is over-production, assertions of absence are more diagnostic than assertions of presence. Thirty of the sixty-four tests in the corrected suite are of this form.
...
\textbf{Limitations.} We enumerate these rather than summarize them. Taken together they mean that the results demonstrate regression and correction behaviour on a single controlled environment, and do not establish general recall or accuracy in any real setting.
...
\textbf{Reproducibility.} The corrected results were first produced by replay, with Docker unavailable, and we characterize that reconstruction precisely before describing the live confirmation. Its active-layer stream is the stream recorded by the earlier live run, relabelled with the provenance the current collector emits and otherwise byte-identical in content. Its web-evidence stream was collected in replay mode against response bodies taken verbatim from the testbed's own served files and from the recorded Nuclei response for the backup directory listing. The identity, secret and manual streams were regenerated by executing the current collectors against local fixtures. The derivation rules, endpoint validation, manifest check, ablation and scoring therefore all ran on real recorded content and are deterministic given that content, and the graph reconstruction is verifiable from the stored artifacts. What has not been demonstrated is that a fresh collection against live containers reproduces these streams. That reconstruction has since been confirmed by a fresh live end-to-end execution (Section~\ref{sec:live}), which reproduces the provenance composition, the semantic composition, the manifest outcome and every path count. The replay is therefore no longer the sole basis for the reported results, though the environment it runs against is unchanged and remains a single controlled testbed.
...
64 tests pass & \texttt{python -m unittest discover -s tests} \\
